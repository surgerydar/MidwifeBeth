...
FileWriter.prototype.writeChunk = function( ws, data ) {
    ...
    if ( this.processed >= this.fileSize ) {
        ...
        let destination = '/media/' + this.filename;
        this.file.once('close', function() {
            if ( !fs.copyFile ) {
                ...
                copyFile( source, './static' + destination ).then( () => {
                    console.log(source + ' was copied to ' + destination);
                    ws.send( JSON.stringify({status:"DONE", guid: self.guid, destination:destination}) );
                }).catch( ( error ) => {
                    console.log('error copying ' + source + ' to ' + destination + ' : ' + error );
                });
            } else {
                fs.copyFile(source, './static' + destination, (error) => {
                    if (error) {
                        console.log('error copying ' + source + ' to ' + destination + ' : ' + error );
                    } else {
                        console.log(source + ' was copied to ' + destination);
